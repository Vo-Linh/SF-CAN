...
class SMDACS(UDADecorator):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        # Generate pseudo-label
        # 1. Get the model's predictions (logits) for the target image using the EMA model.
        ema_logits = self.get_ema_model().encode_decode(target_img, target_img_metas)
        # 2. Convert logits into probabilities using the softmax function.
        ema_softmax = torch.softmax(ema_logits.detach(), dim=1)
        # 3. Get the highest probability (pseudo_prob) and corresponding class label (pseudo_label).
        pseudo_prob, pseudo_label = torch.max(ema_softmax, dim=1)
        # 4. Create a mask for pixels where the probability is greater than or equal to the threshold.
        ps_large_p = pseudo_prob.ge(self.pseudo_threshold).long() == 1
        # 5. Calculate the total number of pixels.
        ps_size = np.size(np.array(pseudo_label.cpu()))
        # 6. Compute the proportion of valid pseudo-labels.
        pseudo_weight = torch.sum(ps_large_p).item() / ps_size
        # 7. Scale the pseudo-weight and create a tensor of the same shape as pseudo_prob.
        pseudo_weight = pseudo_weight * torch.ones(pseudo_prob.shape, device=dev)

        pseudo_label_keep = pseudo_label
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            for j in range(batch_size):
                ...
                #            cmap="cityscapes")
                subplotimg(
                    axs[0][4], mixed_lbl[j], 'Seg Mixed Targ', cmap=self.cmap)
                subplotimg(
